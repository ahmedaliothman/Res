...
function* onUpdateAppRequest({ type, payload }: actionTypes.UpdateActionType) {
  try {
    ...
    if (store.getState().router.location?.pathname.toLowerCase() == "/client/result" || store.getState().router.location?.pathname.toLowerCase() == "/Res/Employee/AddRequestAsClient/ResultAdmin") {
      yield put(RequestClearnewApp());
      yield put(RequestClearPersonalInfo());
      yield  put(RequestClearPassportInfo());
      yield  put(RequestClearattachmentDocuments());
    }
      yield put(pageLoaderActions.getPageLoaderRequest(false));
