...
const PassportInfo = () => {
  ...
  const { register, handleSubmit, watch, errors,setValue, getValues,control } = useForm<IFormData>({
    defaultValues:{
      ...
      civilID: selectedUser?.civilId?Number(selectedUser?.civilId):undefined,
  ...
  useEffect(() => {
    setValue("EmployeeNumber", selectedUser?.employeeNumber);
  ...
  useEffect(  () => {
   if(paci!=undefined && paci !== null)
   {
    ...
    if (paci?.aDDITIONAL_PARAM_2Field!= undefined && paci?.aDDITIONAL_PARAM_2Field != null )
      {
  ...
  const onSubmit = async (data:IFormData) => {
    ...
    res.civilID = selectedUser?.civilId?Number(selectedUser?.civilId):undefined;

    if(Direction=="fwd")
    {
      res.navigationUrl="/Res/Employee/AddRequestAsClient/FileAttachmentsAdmin";
    }
    else if(Direction=="bwd")
    {
      res.navigationUrl="/Res/Employee/AddRequestAsClient/PersonalInfoAdmin";
    }
