...
const NewApp: React.FunctionComponent<INewAppState> =() => {
  ...
  useEffect(() => {
    if(userData?.selectedUser?.civilId ==undefined)
    {
      ...
      history.push("/Res/Employee/AddAdminRequestForUser");
       ...
       {
          dispatch(getFetchIncompleteRequest(userData.selectedUser?.userId as number));
       }
...
useEffect(()=>{
  ...
  newAppState.message="";
	...
	const handleSubmit = async (e: React.SyntheticEvent) => {
     ...
     newAppState.IState.history=history;
     if(Direction == "fwd")
     {
      newAppState.IState.navigationUrl ='/Res/Employee/AddRequestAsClient/PersonalInfoAdmin';

     }
     else
     if (Direction == "bwd")
     {
      newAppState.IState.navigationUrl ='/Res/Employee/AddAdminRequestForUser';
     ...
     if (newAppState.IState?.applicationNumber===undefined||newAppState.IState?.applicationNumber===0)
     {
      ...
      newAppState.IState.userId=userData.selectedUser?.userId;
 ...
 {appType ==undefined && 
                          ...
                          <button type="submit" className="btn btn-primary btn-user  shorooq "  onClick={() => { setDirection("fwd"); }} style={{fontSize: '22px'}}>
                           {labelName}
                            ...
                            <button type="submit" className="btn btn-primary btn-user shorooq  " onClick={() => { setDirection("bwd"); }} style={{ fontSize: '22px', marginRight:"20px" }}>
