...
let columnsBootStrap_: columnsBootStrap={columns:[],data:null,tableName:"" , hasSelection:false};
  columnsBootStrap_.columns=[
    ...
    {
        text: "تعديل",
        formatter:(row:any,cell:any) => <button className="btn btn-warning btn-sm  rounded-circle" disabled={(cell.applicationStatusId!=6 && cell.applicationStatusId!=3)}  onClick={()=>handleRowFunctionsref.current?.EditRowView(cell)} ><i className="fas fa-edit"aria-hidden="true"/></button>
    }
