...
function FileAttachments() {
    ...
    const router = useSelector<RootState, RootState["router"]>(state =>state.router);
    ...
    const onSubmit = async (data: TempFormData) => {
        ...
        if (Direction === "fwd") {
            postData.navigationUrl="/Res/Employee/EditView/Agreament";
          }
          else if (Direction === "bwd") {
            postData.navigationUrl="/Res/Employee/EditView/passportinfo";
          }
        if (stateData.id === undefined || stateData.id === 0) {
            ...
            if (data.ApprovedLetterForResidencyRenewal!=null&&data.ApprovedLetterForResidencyRenewal.length > 0)
                postData.ApprovedLetterForResidencyRenewal = data.ApprovedLetterForResidencyRenewal[0];
            if (data.SalaryCertification!=null&&data.SalaryCertification.length > 0)
                postData.SalaryCertification = data.SalaryCertification[0];
            if (data.CivilIdCopy!=null&&data.CivilIdCopy.length > 0)
                postData.CivilIdCopy = data.CivilIdCopy[0];
            if (data.PassportCopy!=null&&data.PassportCopy.length > 0)
                postData.PassportCopy = data.PassportCopy[0];
            if (data.OtherRelatedDocuments!=null&&data.OtherRelatedDocuments.length > 0)
                postData.OtherRelatedDocuments = data.OtherRelatedDocuments as FileList;
