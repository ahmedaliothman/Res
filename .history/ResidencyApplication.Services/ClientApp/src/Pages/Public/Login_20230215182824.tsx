...
const Login = () => {
	...
	const handleNewUser = async (e: SyntheticEvent) => {
		...
		await dispatch(getHistoryPushRequest({ history: history, pageName: "/Public/register" }));
	...
	return (
		<Layout>
			...
			{(loginstate.userInfo?.userRoleId==1||loginstate.userInfo?.userRoleId==3)&&
				<Redirect
					to={{
						pathname: "/Res/CommonPages/LandingPage",
					}}

				/>
			}
			
			{/* {(loginstate?.isLoggedIn && loginstate.userInfo?.userRoleId == 3) &&
				<Redirect
					to={{
						pathname: "/Res/Employee/AddRequestAsClient/NewAppAdmin",
					}}

				/>
			} */}
			{( loginstate.isLoggedIn==true&&loginstate.userInfo?.userTypeId!=26)&&(loginstate.userInfo?.nationalityId?.toLowerCase()!="kw") &&
				<Redirect
					to={{
						pathname: "/Client/NewApp",
					}}
				/>
			}
		</Layout>
