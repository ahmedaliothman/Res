...
// function FileAttachments() {
    ...
    const onSubmit = async (data: TempFormData) => {
        ...
        if (Direction === "fwd") {
            postData.navigationUrl="/Client/Agreament";
          }
          else if (Direction === "bwd") {
            postData.navigationUrl="/Client/passportinfo";
          }
        if (stateData.id === undefined || stateData.id === 0) {
            ...
            if (data.ApprovedLetterForResidencyRenewal.length > 0)
            {
                ...
                else
                {
                    ...
                    message.message = "برجاء اختيار ملف صحيح" as unknown as string;
                    message.status = ToastMessageStatus.error;
                    toastMessagesActions.getToastMesageRequest(message);
                                                ...
                                                <div className="form-group row">
                                                    ...
                                                    <div className="col-sm-8">
                                                        ...
                                                        {stateData.otherRelatedDocuments?.split(",").map((r,i)=>{
                                                            if(r)
                                                            return  <> <a href={r} target="blank">   ملف رقم{i+1}    </a><button onClick={(e) =>{ e.preventDefault(); setRemoveOtherDocs(true)}}>Edit</button></>
                                                        })}
