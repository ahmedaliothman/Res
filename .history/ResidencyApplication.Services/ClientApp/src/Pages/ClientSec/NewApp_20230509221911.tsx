...
const NewApp: React.FunctionComponent<INewAppState> = () => {
  ...
  const handleSubmit = async (e: React.SyntheticEvent) => {
    ...
    if (newAppState.IState?.applicationNumber === undefined || newAppState.IState?.applicationNumber === 0) {
      ...
      newAppState.IState.navigationUrl = '/Client/personalInfo';
      newAppState.IState.history=history;
      await dispatch(getCreateRequest(newAppState.IState));
      // history.push('/Client/personalInfo');
    }
    else {
      newAppState.IState.remark = "";
      newAppState.IState.navigationUrl = '/Client/personalInfo';
  ...
  const handleAppTypeChange =  (value?: SelectOptions | SelectOptions[] | null) => {
    if (value) {
                      ...
                      <form className="user" onSubmit={handleSubmit}>
                        ...
                        <div className="form-group d-flex">
                          ...
                          <Select
                            ...
                            options={AppTypes}
                            value={appType}
                            onChange={(temp)=> setappType(temp as SelectOptions)}
                            rules={{
                              required: true
                            }}
                          >
